...
func (c *CueInstanceCache) indexExists(revision string) bool {
    ...
    c.db.View(func(tx *buntdb.Tx) error {
        idxs, err := tx.Indexes(); if err != nil {
            indexExists = false;
            return err
        }
...
func (c *CueInstanceCache) createIndex(artifact *sourcev1.Artifact) error {
    if !c.indexExists(artifact.Revision) {
        ...
        return c.db.Update(func (tx *buntdb.Tx) error {
            ...
            // insert timestamp
            tsKey, tsVal := idx.ts()
            _, _, err := tx.Set(tsKey, tsVal, nil); if err != nil {
                return err
            }
...
func (c *CueInstanceCache) gc(keepLast int) error {
    ...
    c.db.View(func(tx *buntdb.Tx) error {
        ...
        for _, idxName := range idxs {
            tss, err := tx.Get(fmt.Sprintf(tsKeyFmt, idxName)); if err != nil {
                return err
            }
            ts, err := time.Parse(time.RFC3339, tss); if err != nil {
                return err
            }
    ...
    sort.Slice(indexes, func(i, j int) bool {
        return indexes[i].timestamp.Before(indexes[j].timestamp)
    })
...
func (c *CueInstanceCache) getInstance(cacheKey string) ([]byte, error) {
    ...
    err := c.db.View(func (tx *buntdb.Tx) error {
        val, err := tx.Get(cacheKey); if err != nil {
            return err
        }
